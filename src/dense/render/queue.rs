...
pub fn tile_grid_dense_queue_draw_commands(
    ...
    r_tile_grid_pipeline: Option<Res<TileGridDensePipeline>>,
    q_tile_grid: Query<&PreparedTileGridDense, Without<RenderVisibleEntities>>,
    ...
    mut views: Query<
        ...
        Without<PreparedTileGridDense>
    >,

    mut next_tick: Local<Tick>,
) {
    let Some(tile_grid_dense_pipeline) = r_tile_grid_pipeline else { return; };
    let draw_tile_grid_dense_transparent = draw_functions_transparent.read().id::<TileGridDenseDrawCommands>();
    let draw_tile_grid_dense_mask        = draw_functions_mask.read().id::<TileGridDenseDrawCommands>();
    let draw_tile_grid_dense_opaque      = draw_functions_opaque.read().id::<TileGridDenseDrawCommands>();
    ...
    for (visible_entities, view, msaa) in &mut views {
        ...
        for &(render_entity, main_entity) in visible_entities.get::<TileGridDenseInfo>() {
            ...
            let pipeline_id = pipelines.specialize(
                &pipeline_cache,
                &tile_grid_dense_pipeline,
                TileGridDensePipelineKey{
                    mesh_key,
            ...
            match grid.render_pass {
                RenderPass2d::Opaque => {
                    ...
                    phase_opaque.add(
                        ...
                        Opaque2dBinKey{
                            ...
                            draw_function: draw_tile_grid_dense_opaque,
                ...
                RenderPass2d::AlphaMask => {
                    ...
                    phase_mask.add(
                        ...
                        AlphaMask2dBinKey{
                            pipeline: pipeline_id,
                            draw_function: draw_tile_grid_dense_mask,
                            asset_id: AssetId::<Mesh>::invalid().untyped(),
                            material_bind_group_id: None,
                        },
                        (render_entity, main_entity),
                ...
                RenderPass2d::Transparent => {
                    ...
                    phase_transparent.add(Transparent2d {
                        draw_function: draw_tile_grid_dense_transparent,
                        pipeline: pipeline_id,
