...
use crate::{atlas::TileAtlas, dense::{render::{PreparedTileGridDense, TileGridDenseBindGroups}, TileGridDenseData, TileGridDenseInfo}, shared::{RenderPass2d, TileGridAnimator}};
...
#[derive(Debug, Component)]
pub struct TileGridDenseExtractedUpdate {
...
pub fn tile_grid_dense_extract_updates(
    ...
    q_extracted: Extract<Query<(&RenderEntity, &TileGridDenseData, &TileGridDenseInfo, Option<&TileGridAnimator>, &GlobalTransform, &ViewVisibility)>>,
    q_tile_grid_dense_s: Query<&TileGridDenseExtracted>,
    r_tex_atlases: Extract<Res<Assets<TileAtlas>>>,
    image_assets:   Res<RenderAssets<GpuImage>>,
) {
    q_extracted.iter().for_each(|(entity, grid_data, grid_info, animator, transform, visibility)| {
        if !visibility.get() { return; }

        let uniforms = TileGridDenseUniforms {
            origin:         transform.translation().xy() + grid_info.offset() * grid_info.scale(),
            depth:          transform.translation().z,
            size:           grid_info.size(),
            scale:          grid_info.scale(),
            y_depth_scale:  grid_info.y_depth_scale(),
            frame_time:     animator.map_or(0, TileGridAnimator::frame_count),
        };

        let render_pass = grid_info.render_pass();
        let texture = grid_info.atlas().as_ref()
        ...
        let (update_data, update_uniforms, update_texture, update_blend) = if let Ok(dst) = q_tile_grid_dense_s.get(entity.entity()) {
            (
                dst.data_change_tick != grid_data.data_change_tick(),
        ...
        commands
            ...
            .insert((
                TileGridDenseExtractedUpdate{
                    data:     update_data.then(|| bytemuck::cast_slice_box(grid_data.data_clone())),
                ...
                TileGridDenseExtracted{
                    texture, 
                    data_change_tick: grid_data.data_change_tick(),
                    uniforms,
                    render_pass,
                }
            ));
