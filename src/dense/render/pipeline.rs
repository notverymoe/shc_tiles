...
use super::HANDLE_TILE_GRID_DENSE_SHADER;

#[derive(Debug, Resource)]
pub struct TileGridDensePipeline {
    pub(super) shader:         Handle<Shader>,
    pub(super) layout_view:    BindGroupLayout,
    pub(super) layout_tilemap: BindGroupLayout,
}

pub fn tile_grid_dense_pipeline_init(mut commands: Commands) {
    commands.init_resource::<TileGridDensePipeline>();
}

impl FromWorld for TileGridDensePipeline {
    fn from_world(render_world: &mut World) -> Self {
        let render_device = render_world.get_resource::<RenderDevice>().unwrap();
        
        Self { 
            shader: HANDLE_TILE_GRID_DENSE_SHADER,
...
pub struct TileGridDensePipelineKey {
    pub mesh_key: Mesh2dPipelineKey,
    pub blend:    Option<BlendState>,
}

impl SpecializedRenderPipeline for TileGridDensePipeline {
    type Key = TileGridDensePipelineKey;
